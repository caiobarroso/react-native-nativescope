...
If your app needs to install the bridge imperatively — for example because the `QueryClient` lives
somewhere the automatic discovery cannot reach — do it behind a `__DEV__` guard.

```tsx
import { getQueryClient } from "@/infrastructure/reactQuery/query-client";

if (__DEV__) {
  const { installNativeScopeDevtools } = require("react-native-nativescope/app");
  installNativeScopeDevtools({
    modules: { storage: { reactQuery: getQueryClient() } },
  });
}
```

Use `require` rather than a top-level `import` so the module is never part of the production module
graph.
