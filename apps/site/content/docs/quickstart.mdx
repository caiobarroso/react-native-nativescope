...
through React Query, the cache can keep showing the old one until something invalidates it. A small,
optional config file closes that gap — see the [React Query bridge](/docs/react-query). Neither it
nor the in-app [indicator](/docs/configuration) is needed for storage discovery.
...
config. [Devices & connection](/docs/devices#bring-your-own-metro) covers that setup, when to choose
