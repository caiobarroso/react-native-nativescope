...
For `@op-engineering/op-sqlite`, use `useInspectedOpSqlite` — same signature, and it reads rows
through that driver's `execute`:

```tsx
const { rows, loading, reload } = useInspectedOpSqlite<Photo>(
  db,
  "select * from photos where album = ?",
  ["trips"],
);
```
