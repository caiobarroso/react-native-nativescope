---
title: Configuration
description: The optional nativescope.config.ts file, and when you actually need it.
---

Core storage discovery, inspection and editing work with no configuration at all. You only add a
config file when a module needs app-side behavior that cannot be inferred safely. Today, the
Storage module exposes an in-app indicator and cache invalidation for a layer like React Query.
...
Covered in depth in the [React Query bridge](/docs/react-query). The short version:
