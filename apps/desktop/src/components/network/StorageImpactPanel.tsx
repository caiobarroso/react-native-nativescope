...
import {
  groupImpact,
  type StorageImpactItem,
} from "../../lib/network-storage-link.ts";
...
export function StorageImpactPanel({ items }: { items: StorageImpactItem[] }) {
    ...
    <section className="flex min-h-0 flex-1 flex-col">
      ...
      {items.length > 0 ? (
        ...
        <p className="text-[11px] text-text-subtle">
          No storage changes right after this response.
        </p>
      ) : (
        <EnableStorageCard />
      )}
...
function ImpactChips({ items }: { items: StorageImpactItem[] }) {
  ...
  return (
    <div className="flex flex-col gap-2">
      {groups.map((group) => (
        <div
          key={`${group.providerId} ${group.instanceId}`}
  ...
  return (
    ...
    >
      ...
      <ArrowUpRight
        size={11}
        strokeWidth={1.5}
        className="shrink-0 text-text-subtle group-hover:text-accent"
      />
    </button>
  );
}
      ...
      {open && (
        <div className="mt-2">
          <div className="relative">
            ...
            >
              {copied ? (
                <Check size={10} className="text-created" />
              ) : (
                <Copy size={10} />
              )}
              {copied ? "Copied" : "Copy"}
